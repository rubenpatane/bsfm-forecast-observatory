...
7. reruns the full test, registry and scientific-audit chain after acquisition;
8. evaluates the fail-closed model lifecycle and final scientific readiness;
9. generates the public machine-readable state: readiness, G1-G4 evidence inventory, provenance-gated refinements and real acquired-data statistics/recent FAA Boeing reports;
10. uploads the normalized NTSB research artifact;
11. commits changed generated state back to `main` through the bounded rebase/retry workflow without force-pushing;
12. packages and deploys the static observatory to GitHub Pages.
...
The implementation is live and runtime-verified through AGGIORNA #16. No additional AGGIORNA is required merely to verify this batch. Future AGGIORNA runs are appropriate when official source data should be refreshed or when repository changes need integrated verification/deployment.

Exact next step: continue accumulating admissible evidence for G1-G4 while using AGGIORNA as the repeatable refresh/audit/publication pipeline. Do not alter the frozen F-002 forecast retrospectively.
