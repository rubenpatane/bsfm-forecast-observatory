...
def inspect_faa_csv(raw):
 ...
 fields=set(rd.fieldnames or []); rows=boeing=bad_dates=0; dates=[]
 for r in rd:
...
def ingest_faa_sdr(year):
 ...
 d={'schema':'bsfm.source-manifest.v1','source':'FAA SDR','year':year,'official_url':FAA.format(year=year),'retrieved_at':now(),'sha256':sha(raw),'bytes':len(raw),**stats,'status':'validated' if valid else 'invalid','validation':['http_download_ok','csv_parse_ok','required_schema','difficulty_date_parse']}
 return manifest(f'faa-sdr-{year}.json',d)

def ingest_ntsb_metadata():
 raw=fetch(NTSB); is_zip=raw[:4]==b'PK\x03\x04'
 d={'schema':'bsfm.source-manifest.v1','source':'NTSB AVALL','official_url':NTSB,'retrieved_at':now(),'sha256':sha(raw),'bytes':len(raw),'status':'validated' if is_zip and len(raw)>1024 else 'invalid','validation':['http_download_ok','zip_signature','nontrivial_size'],'note':'Archive fingerprint only. MDB tables are exported in the dedicated historical source build.'}
