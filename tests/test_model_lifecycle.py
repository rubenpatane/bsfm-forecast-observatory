...
def test_candidate_gate_blocks_unverified_point_in_time_from_foundation_report():
    source={'source_integrity_ready':True,'point_in_time_availability_verified':True}
    report=full_report(); report['point_in_time_availability_verified']=False
...
def test_fit_gate_is_not_circular_on_postfit_calibration():
    source={'source_integrity_ready':True}
    report={'point_in_time_availability_verified':True,'leakage_free':True,'baseline_present':True,'historical_cases':True}
...
def test_fit_gate_still_requires_source_integrity():
    source={'source_integrity_ready':False}
    gate=fit_gate(source,full_report())
    assert not gate['pass'] and 'source_integrity_ready' in gate['missing']
...
def test_fit_candidate_is_blocked_without_evidence_and_fits_when_gate_open():
    blocked=fit_candidate({'source_integrity_ready':True},{},[],[],['777'])
